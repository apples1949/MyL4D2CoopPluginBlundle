...
public void OnPluginStart()
{
	...
	g_hCvarEffects =	CreateConVar(	"l4d_tree_effects",			"15",			"0=关闭。1=灯光,2=球,4=火花,8=溶解(礼物),16=烟雾(礼物-可能导致服务器罕见的崩溃),31=全部。数字相加", CVAR_FLAGS );
	g_hCvarGifts =		CreateConVar(	"l4d_tree_gifts",			"6",			"在树下产生多少个包裹", CVAR_FLAGS );
...
Action CmdSpawnerTemp(int client, int args)
{
	if( !client )
	{
		ReplyToCommand(client, "[Trees] 指令只能在%s使用.", IsDedicatedServer() ? "专用服务器的游戏中" : "监听服务器的聊天框中");
...
Action CmdSpawnerDel(int client, int args)
{
	if( !g_bCvarAllow )
	{
		ReplyToCommand(client, "[Trees] 插件已关闭.");
		return Plugin_Handled;
	}
...
Action CmdSpawnerReload(int client, int args)
{
	...
	ResetPlugin();
	CreateTimer(0.1, TimerStart, _, TIMER_FLAG_NO_MAPCHANGE);
	ReplyToCommand(client, "[Trees] 已重载和重启配置.");
...
Action CmdSpawnerGlow(int client, int args)
{
	static bool glow;
	glow = !glow;
	PrintToChat(client, "%s是否启用生成 %s", CHAT_TAG, glow ? "启用" : "禁用");
...
Action CmdSpawnerTele(int client, int args)
{
	if( args == 1 )
	{
		...
		PrintToChat(client, "%s无法找到传送的指标.", CHAT_TAG);
	}
	else
		PrintToChat(client, "%sUsage: sm_tree_tele <指标 1-%d>.", CHAT_TAG, MAX_SPAWNS);
	return Plugin_Handled;
}
	...
	if( aim != -1 )
	{
		...
		for( int i = 0; i < MAX_SPAWNS; i++ )
		{
			...
			if( entity == aim  )
			{
				...
				PrintToChat(client, "%s新角度: %f %f %f", CHAT_TAG, vAng[0], vAng[1], vAng[2]);
				break;
			}
		}
	}
	...
	if( aim != -1 )
	{
		...
		for( int i = 0; i < MAX_SPAWNS; i++ )
		{
			...
			if( entity == aim  )
			{
				...
				PrintToChat(client, "%s新位置: %f %f %f", CHAT_TAG, vPos[0], vPos[1], vPos[2]);
				break;
			}
		}
	}
