...
public void OnClientConnected(int client)
{
	...
	g_iPlayerNumber += 1;
	IsPlayerMultiple(true, true, true, client, GetAliveSurvivalMaxNumber(), GetMaxPlayers());
}
...
public void OnClientDisconnect(int client)
{
	...
	g_iPlayerNumber -= 1;
	IsPlayerMultiple(true, false, true, client, GetAliveSurvivalMaxNumber(), GetMaxPlayers());
}
...
public Action IsCreateMoreMedicalTimer(Handle Timer, DataPack hPack)
{
	...
	IsPlayerMultiple(false, false, g_bMoreCheck, 0, GetAliveSurvivalMaxNumber(), GetMaxPlayers());
...
public void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		IsPlayerMultiple(false, false, true, 0, GetAliveSurvivalMaxNumber(), GetMaxPlayers());
	}
}
...
// 是否播放声音 玩家是否连接或断开 额外检测 玩家索引 玩家数量 最大玩家位置
...
// 玩家索引 补给倍数 是否播放声音 玩家连接或断开 额外检测 玩家数量 最大玩家位置
void IsUpdateEntCount(int client, int g_Multiple, bool g_bPrompt, bool g_bContent, bool g_bMoreCheck, int g_iClientNumber, int g_iSurvivorLimit)
{
	...
	{	 // g_iPlayerNumber 当前玩家数量 g_iSurvivorLimit 最大位置 g_iClientNumber 生还者数量
		if (g_bPrompt)
			if (!g_bPlayerPrompt)
			...
			PrintToChatAll("\x04[提示]\x05当前生还者数量为\x03:%i\x03,\x05更改为\x03%s\x05倍医疗物品.", g_iClientNumber, g_sMedical);
	}
}
