...
public void OnPluginStart()
{
	...
	g_hCvarDistGive =		CreateConVar(	"l4d_gear_transfer_dist_give",		"150.0",		"bot在多少范围内会给于玩家物品", CVAR_FLAGS);
	g_hCvarDistGrab =		CreateConVar(	"l4d_gear_transfer_dist_grab",		"150.0",		"bot在多少范围内会拾取物品", CVAR_FLAGS);
	...
	g_hCvarStart =			CreateConVar(	"l4d_gear_transfer_start",			"0.0",			"在回合开始时的指定时间内阻止自动抓取/给予.", CVAR_FLAGS);
	...
	g_hCvarTimeout =		CreateConVar(	"l4d_gear_transfer_timeout",		"5.0",			"超过多少时间阻止bot在与玩家交换物品后归还物品(returning an item)和防止bot自动拾取附近掉落的物品", CVAR_FLAGS, true, 1.0);
	g_hCvarTraces =			CreateConVar(	"l4d_gear_transfer_traces",			"15",			"每帧自动给予或获取的最大次数(ray traces per frame). 此值可以增加但效果提升很小", CVAR_FLAGS, true, 1.0, true, 120.0);
	g_hCvarGive =			CreateConVar(	"l4d_gear_transfer_types_give",		"123456789",	"bot可以给予什么物品. 0=关. 1=肾上腺素, 2=药丸, 3=燃烧瓶, 4=手雷, 5=胆汁, 6=医疗包, 7=高爆弹药包, 8=燃烧弹药包, 9=电击器. 任意数字组合选择物品", CVAR_FLAGS);
	g_hCvarGrab =			CreateConVar(	"l4d_gear_transfer_types_grab",		"123456789",	"bot可以拾取什么物品. 0=关. 1=肾上腺素, 2=药丸, 3=燃烧瓶, 4=手雷, 5=胆汁, 6=医疗包, 7=高爆弹药包, 8=燃烧弹药包, 9=电击器. 任意数字组合选择物品", CVAR_FLAGS);
	g_hCvarTypes =			CreateConVar(	"l4d_gear_transfer_types_real",		"123456789",	"玩家可以交换什么物品. 0=关. 1=肾上腺素, 2=药丸, 3=燃烧瓶, 4=手雷, 5=胆汁, 6=医疗包, 7=高爆弹药包, 8=燃烧弹药包, 9=电击器. 任意数字组合选择物品.", CVAR_FLAGS);
	g_hCvarVocalize =		CreateConVar(	"l4d_gear_transfer_vocalize",		"1",			"0=关. 1=玩家给于物品时发出声音. 每关最开始的60秒无声音.", CVAR_FLAGS);
	CreateConVar(							"l4d_gear_transfer_version",		PLUGIN_VERSION, "插件版本号", FCVAR_NOTIFY|FCVAR_DONTRECORD);
