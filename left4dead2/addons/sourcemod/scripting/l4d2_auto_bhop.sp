...
ConVar cv_autoBhopPlayerDefault;
	...
	OnPluginStart()
{
	...
	cv_autoBhopPlayerDefault = CreateConVar("l4d2_auto_bhop_player_default", "1", "If 0, the player enters the server with bhop disabled by default.");
...
void Event_PlayerConnect(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (GetConVarInt(cv_autoBhopPlayerDefault) != 0) g_AutoBhop[client] = true;
	else g_AutoBhop[client] = false;
}
