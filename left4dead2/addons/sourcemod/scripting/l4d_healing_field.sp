...
public void OnPluginStart()
{
	...
	hCvar_MaxHealHP				= CreateConVar("l4d_healing_field_MaxHp",		"150",		"治疗阵最多治疗到多少血", FCVAR_NOTIFY, true, 100.0, true, 9999.0);
	...
	hCvar_MaxHealHP.AddChangeHook(Event_ConVarChanged);
...
void GetCvars()
{
	...
	iCvar_MaxHealHP = hCvar_MaxHealHP.IntValue;
}
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		if( IsClientInGame( i ) && GetClientTeam( i ) == 2 && IsPlayerAlive( i ) )
		{
			...
			if( GetVectorDistance( vPos, vEnd ) <= fCvar_HealingFieldRange )
			{
				...
				if( iHealth < iCvar_MaxHealHP )
				{
//					iHealth += RoundFloat( 6.0 );
					iHealth += iCvar_AmountHealth; // Add the extra amount wants to the current amount.
					if( iHealth > iCvar_MaxHealHP )
						iHealth = iCvar_MaxHealHP;

					fHealth = GetTempHealth( i );
					if( iHealth + fHealth > iCvar_MaxHealHP )
					{
						fHealth = hCvar_MaxHealHP.FloatValue - iHealth;
