...
{
	g_hGravesEnabled 	= CreateConVar("l4d_graves_enable", "1", "是否启用这个插件", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hGraveNotSolid 	= CreateConVar("l4d_graves_not_solid", "0", "是否显示固定形象的坟墓", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hGraveDelay 		= CreateConVar("l4d_graves_delay", "5.0", "幸存者死亡后多久生成坟墓", FCVAR_NOTIFY, true, 1.0);
	g_hGraveGlow 		= CreateConVar("l4d_graves_glow", "1", "坟墓是否发光", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hGraveGlowColor 	= CreateConVar("l4d_graves_glow_color", "-1 -1 -1", "坟墓发光RGB颜色 仅L4D2[-1 -1 -1: 随机颜色]", FCVAR_NOTIFY);
	g_hGraveGlowRange 	= CreateConVar("l4d_graves_glow_range", "1500", "坟墓发光范围", FCVAR_NOTIFY, true, 1.0);
	g_hGraveHealth 		= CreateConVar("l4d_graves_health", "1500", "坟墓承受多少伤害后破碎 (L4D2中, 0表示坟墓不会被破坏)", FCVAR_NOTIFY, true, 0.0);
