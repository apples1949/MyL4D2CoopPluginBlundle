...
ConVar g_ConVarEnable,
	g_ConVarChanceFM, g_ConVarMinDelay, g_ConVarMaxDelay, g_ConVarSpawnInterval, g_ConVarCount, g_ConVarCountLimit, g_ConVarCountMode, g_ConVarControlHP, g_ConVarHP, g_ConVarVoteAccess, g_hCvarAnnounceDelay;
...
public void OnPluginStart()
{
	...
	g_ConVarChanceFM				 = CreateConVar("l4d_tank_on_spawn_fm_chance", "0", "坦克在第一关出现的概率(1-100), 0-关闭.", CVAR_FLAGS);
	...
	g_ConVarVoteAccess				 = CreateConVar("l4d_tank_on_spawn_voteaccess", "z", "发起投票所需要的权限标志(留空则所有人都可以投票)", CVAR_FLAGS);
	...
	g_hCvarLog						 = CreateConVar("l4d_tank_on_spawn_log", "0", "是否开启日志记录? (1-开启/0-关闭)", CVAR_FLAGS);
	...
	g_hCvarAddTanksOnEasy			 = CreateConVar("l4d_tank_on_spawn_add_tanks_easy", "-1", "游戏难度为简单时额外增加多少只坦克 (可以使用负数数值)", CVAR_FLAGS);
	...
	g_hCvarIngoneRush				 = CreateConVar("l4d_tank_on_spawn_ignore_rush", "0", "如果有玩家在上一波坦克没有被击杀的情况下跑图，那么关闭双倍坦克吗？(Do not double tanks if somebody rushed when previous tank wave is not yet killed?) (1-是 / 0否)", CVAR_FLAGS);
	...
	g_hCvarDirectorWavesInterval_Fin = CreateConVar("l4d_tank_on_spawn_director_waves_interval_fin", "1", "(地图最后一关) 在上一个坦克死亡后，允许导演生成新的坦克之前所需的最小时间（秒）", CVAR_FLAGS);
	g_hCvarDirectorWavesInterval_Min = CreateConVar("l4d_tank_on_spawn_director_waves_interval_min", "1", "((地图最后一关之外) 在最后一个坦克死亡后，允许导演产生新的坦克之前所需的最小随机开始时间（秒）", CVAR_FLAGS);
	g_hCvarDirectorWavesInterval_Max = CreateConVar("l4d_tank_on_spawn_director_waves_interval_max", "1", "(地图最后一关之外) 在允许导演在最后一个坦克死亡后产生新的坦克之前所需的最小随机结束时间（秒）", CVAR_FLAGS);
	g_hCvarMapStartRelax			 = CreateConVar("l4d_tank_on_spawn_mapstart_relax_time", "1", "防止导演系统在地图开始后的这个秒数内产生坦克", CVAR_FLAGS);
	...
	RegConsoleCmd("sm_tsm", Command_MenuTankSpawn, "Show tank spawn menu");
...
public Action Command_Veto(int client, int args)
{
	...
	{	 // IsVoteInProgress() is not working here, sm bug?
		...
		*/
		...
		if (g_ConVarControlHP.BoolValue || g_bApplyMenuHealth)
		{
			...
			if (g_ConVarAnnouncement.BoolValue)
			{
				...
				PrintToChatAll("[提示]已生成 %i 坦克 总共 %i 坦克", iTanksCount, iTanksPlanned);
			}
		}
...
bool IsFirstMap_Old()
{
	if (g_bLeft4Dead2)
	{
		if (0 == strcmp(g_sMap, "c1m1_hotel", false) || 0 == strcmp(g_sMap, "c2m1_highway", false) || 0 == strcmp(g_sMap, "c3m1_plankcountry", false) || 0 == strcmp(g_sMap, "c4m1_milltown_a", false) || 0 == strcmp(g_sMap, "c5m1_waterfront", false) || 0 == strcmp(g_sMap, "c6m1_riverbank", false) || 0 == strcmp(g_sMap, "c7m1_docks", false) || 0 == strcmp(g_sMap, "c8m1_apartment", false) || 0 == strcmp(g_sMap, "c9m1_alleys", false) || 0 == strcmp(g_sMap, "c10m1_caves", false) || 0 == strcmp(g_sMap, "c11m1_greenhouse", false) || 0 == strcmp(g_sMap, "C12m1_hilltop", false) || 0 == strcmp(g_sMap, "c13m1_alpinecreek", false) || 0 == strcmp(g_sMap, "c14m1_junkyard", false))
		{
			return true;
		}
		return false;
		...
		if (0 == strcmp(g_sMap, "l4d_hospital01_apartment", false) || 0 == strcmp(g_sMap, "l4d_garage01_alleys", false) || 0 == strcmp(g_sMap, "l4d_smalltown01_caves", false) || 0 == strcmp(g_sMap, "l4d_airport01_greenhouse", false) || 0 == strcmp(g_sMap, "l4d_farm01_hilltop", false) || 0 == strcmp(g_sMap, "l4d_river01_docks", false))
		{
			return true;
		}
		return false;
